...
The average normalized core size is close to 1/3, as the limiting distribution predicts. However, the relatively large CV shows that individual networks can have core sizes far from the average, so the mean alone can't represent a typical single realization.